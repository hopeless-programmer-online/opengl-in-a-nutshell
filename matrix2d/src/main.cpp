...
layout (location = 0) uniform mat3x2 transformation;
...
void main() {
    gl_Position = vec4((transformation * vec3(inPosition, 1)).xy, 0, 1);
...
int main() {
    try {
        ...
        while (!glfwWindowShouldClose(window))
        {
            ...
            glm::mat3x2 transformation = glm::transpose(glm::mat2x3(
                 glm::cos(x), glm::sin(x), glm::sin(0.5f * x) * 0.5f,
                -glm::sin(x), glm::cos(x), glm::cos(0.5f * x) * 0.5f
            ));

            x += glm::radians(1.0f);

            glProgramUniformMatrix3x2fv(program, 0, 1, GL_FALSE, glm::value_ptr(transformation));
